...
def plot_ilvpadst(plotdir,noistks,dspec4,frbdat,fmhzarr,tmsarr,xlim,rmarr,fsize):
	...
	ax1.errorbar(tmsarr, frbdat.phits, frbdat.dphits, fmt='ro', markersize=3, lw=0.5, capsize=1, zorder=8)
	ax1.set_xlim(xlim)
	...
	ax2.errorbar(rmarr[:,2], rmarr[:,3], xerr=(rmarr[:,2]-rmarr[:,0], rmarr[:,1]-rmarr[:,2]), yerr=rmarr[:,4], fmt='co', fillstyle='none', markersize=5, lw=0.5, capsize=1, zorder=8)
	ax2.set_xlim(xlim)
	#ax2.set_ylim([-100,100])
	ax2.set_ylabel(fr'RM (rad m$^{-2})$')
